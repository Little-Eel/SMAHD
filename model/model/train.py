...
from layer import SMART,GCNConv_Encoder,GCNConv_Decoder,GATConv_Encoder,GATConv_Decoder
...
def train_SMART(features,
    ...
    model = SMART(hidden_dims=hidden_dims,device=train_device,Conv_Encoder=Conv_Encoder,Conv_Decoder=Conv_Decoder)
  
    features_cat=torch.cat(features, dim=1)
    
    data = Data(x=features_cat,edge_index=edge)
    cluster_data = ClusterData(data, num_parts=int(np.ceil(data.num_nodes/train_batch_size)) * 10, recursive=False, log=True)

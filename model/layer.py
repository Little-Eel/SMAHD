...
class SMAHD(torch.nn.Module):
    ...
    def forward(self, features, edge_index):
        x = [encoder(feature,edge_index) for encoder,feature in zip(self.encoders,features)]
        if len(x)==1:
            z=self.fc(x[0])
        else:
            z=self.fc(torch.cat(x, dim=1))
        x_rec = [decoder(z,edge_index) for decoder,feature in zip(self.decoders,features)]
        return z, x_rec
